...
odf.OdfUtils = function OdfUtils() {
  var textns = odf.Namespaces.textns, drawns = odf.Namespaces.drawns, xlinkns = odf.Namespaces.xlinkns, whitespaceOnly = /^\s*$/, domUtils = new core.DomUtils, odfNodeNamespaceMap = [odf.Namespaces.dbns, odf.Namespaces.dcns, odf.Namespaces.dr3dns, odf.Namespaces.drawns, odf.Namespaces.chartns, odf.Namespaces.formns, odf.Namespaces.numberns, odf.Namespaces.officens, odf.Namespaces.presentationns, odf.Namespaces.stylens, odf.Namespaces.svgns, odf.Namespaces.tablens, odf.Namespaces.textns];
  function isImage(e) {
...
odf.StyleTree.Tree;
(function() {
  var fons = odf.Namespaces.fons, stylens = odf.Namespaces.stylens, textns = odf.Namespaces.textns, stylemap = {1:"decimal", "a":"lower-latin", "A":"upper-latin", "i":"lower-roman", "I":"upper-roman"};
  odf.ListStyleToCss = function ListStyleToCss() {
    ...
    function getNumberRule(node) {
      var style = node.getAttributeNS(stylens, "num-format"), suffix = node.getAttributeNS(stylens, "num-suffix") || "", prefix = node.getAttributeNS(stylens, "num-prefix") || "", content = "";
      if(prefix) {
    ...
    function addListStyleRule(styleSheet, name, node, itemRule) {
      var selector = 'text|list[text|style-name="' + name + '"]', level = node.getAttributeNS(textns, "level"), listItemRule, listLevelProps = (node.getElementsByTagNameNS(stylens, "list-level-properties")[0]), listLevelPositionSpaceMode = listLevelProps.getAttributeNS(textns, "list-level-position-and-space-mode"), listLevelLabelAlign = (listLevelProps.getElementsByTagNameNS(stylens, "list-level-label-alignment")[0]), listIndent, textAlign, bulletWidth, labelDistance, bulletIndent, followedBy, leftOffset;
...
(function() {
  ...
  gui.ZoomHelper = function() {
    ...
    this.setZoomableElement = function(element) {
      ...
      parentElement = (zoomableElement.parentElement);
      applyDetailedTransform();
...
gui.GuiStepUtils = function GuiStepUtils() {
  var odfUtils = new odf.OdfUtils, stepUtils = new odf.StepUtils, domUtils = new core.DomUtils;
...
gui.Caret = function Caret(cursor, avatarInitiallyVisible, blinkOnRangeSelect) {
  ...
  function updateOverlayHeightAndPosition() {
    var selectionRect = getSelectionRect(), caretStyle;
...
gui.SelectionController = function SelectionController(session, inputMemberId) {
  var odtDocument = session.getOdtDocument(), domUtils = new core.DomUtils, odfUtils = new odf.OdfUtils, baseFilter = odtDocument.getPositionFilter(), keyboardMovementsFilter = new core.PositionFilterChain, rootFilter = odtDocument.createRootFilter(inputMemberId), TRAILING_SPACE = odf.WordBoundaryFilter.IncludeWhitespace.TRAILING, LEADING_SPACE = odf.WordBoundaryFilter.IncludeWhitespace.LEADING, PREVIOUS = core.StepDirection.PREVIOUS, NEXT = core.StepDirection.NEXT;
  ...
  function moveCursorByLine(direction, extend) {
    var paragraphNode = odtDocument.getParagraphElement(odtDocument.getCursor(inputMemberId).getNode()), steps, lineDirection = direction === NEXT ? 1 : -1;
  ...
  function moveCursorToLineBoundary(direction, extend) {
    var lineDirection = direction === core.StepDirection.NEXT ? 1 : -1, steps = odtDocument.getCursor(inputMemberId).getStepCounter().countStepsToLineBoundary(lineDirection, keyboardMovementsFilter);
    if(extend) {
      extendCursorByAdjustment(steps)
    }else {
      moveCursorByAdjustment(steps)
...
(function() {
  ...
  gui.SessionController = function SessionController(session, inputMemberId, shadowCursor, args) {
    ...
    this.destroy = function(callback) {
      var destroyCallbacks = [drawShadowCursorTask.destroy, redrawRegionSelectionTask.destroy, directFormattingController.destroy, inputMethodEditor.destroy, eventManager.destroy, hyperlinkClickHandler.destroy, hyperlinkController.destroy, metadataController.destroy, textController.destroy, destroy];
      if(iOSSafariSupport) {
...
odf.StyleCache = function(odfroot) {
  ...
  function getTextStyleChain(element, chain) {
    var stylename = styleInfo.getStyleName("text", element), parent = element.parentElement;
    if(stylename !== undefined) {
      chain.push("text");
      chain.push(stylename)
    }
